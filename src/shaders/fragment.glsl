...
float getWater(vec3 p) {
    float d = p.y + 5.0 * sin(uTime) + 80.0;
    d += 6.0 * noise(p.xz * 0.02 + uTime);
    return d;
}
...
const vec3 lightPos = vec3(1000.0, 400.0, -1200.0);
...
vec3 getLight(vec3 p, vec3 rd) {
    ...
    float shadow = getSoftShadow(p, l);
    float occ = getAmbientOcclusion(p, normal);
    return (ambient * occ + (specular * occ + diff) * shadow) * vec3(1.0);
}
...
vec3 getSky(vec3 rd) {
    vec3 col = vec3(0.0);
    float sun = 0.01 / max(0.001, 1.0 - dot(rd, normalize(lightPos)));
...
vec3 render(vec2 uv) {
    vec3 col = vec3(0.0);
    vec3 ro = vec3(220.0, 50.0 * sin(uTime * 0.5) + 50.0, 220.0);
    ro.xz *= rot(uTime * 0.15);
    ...
    if(dist < MAX_DIST) {
        col += getLight(p, rd);
        col = mix(getSky(rd), col, exp(-0.0000007 * dist * dist));
    } else {
        col += getSky(rd);
    }

    float geoMask = smoothstep(2.2, 0.2, length(vPosition.xy));
    col *= mix(vec3(0.9), uColor, 0.35);
    col = mix(col * 0.15, col, geoMask);

    return col;
}

void main() {
    vec2 uv = (2.0 * gl_FragCoord.xy - uResolution.xy) / max(1.0, uResolution.y);
    vec3 color = render(uv);
    gl_FragColor = vec4(pow(max(color, vec3(0.0)), vec3(1.5)), 1.0);
}
